...
private:
	...
	void TestImageEffect( Texture* dstTex, Texture* srcTex, Shader* shader, RenderBuffer* ubo ) const;
	void TestBloomImageEffect( Texture* dstTex, Texture* normalColor, Texture* bloomTex, Shader* shader, RenderBuffer* ubo ) const;
	void CreateTestMaterial();

	// sd a09
	void CreateImageEffectObj();
	void UpdateImageEffectObj();
...
public:
	...
	float m_grayStrength;
	Rgba8 m_targetColor;
	float m_grayTint;
	float m_grayTintPow;
	Shader* m_bloomShader = nullptr;
	Shader* m_grayShader = nullptr;
