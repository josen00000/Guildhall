...
float2 ComputeLightFactor( light_t light, float3 world_pos, float3 world_normal, float3 point_to_eye_dir ){
	float light_intensity = CalculateLightIntensity( light, world_pos );
	float light_dist = distance( world_pos, light.world_position );
	...
	float half_direction = normalize( ( point_to_light_dir + point_to_eye_dir ) / 2.f );
	float specular_strength = pow( max( dot( half_direction, world_normal ), 0.0f ), SPECULAR_POWER );
...
float3 ComputeLightAt( float3 world_pos, float3 world_normal, float3 surf_color ) {
	...
	for( int i = 0; i < MAX_LIGHT; i++ ) {
		...
		float tempLightIntensity = CalculateLightIntensity( temp_light, world_pos );
		...
		diffuse += light_factors.x * light.color_intensity.xyz;
		specular += light_factors.y * light.color_intensity.xyz;
	}
