...
{
    float4 color = tDiffuse.Sample( sSampler, input.uv + float2( sin(SYSTEM_TIME_SECONDS), cos(SYSTEM_TIME_SECONDS) ) * .2f );
    return float4( float3( 1.f, 1.f, 1.f ) - color.xyz, color.w );
    //return color; 
}