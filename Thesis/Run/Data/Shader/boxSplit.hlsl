...
cbuffer box_constants : register(b5) {
    float2 BOT_LEFT;
    float2 TOP_RIGHT;
}
...
// Function
//--------------------------------------------------------------------------------------   
float4 ComputePixelData( float4 pos ) {
    if( pos.x > BOT_LEFT.x && pos.x < TOP_RIGHT.x && pos.y < BOT_LEFT.y && pos.y > TOP_RIGHT.y ) {
...
{
    float4 color = ComputePixelData( input.position );
