...
constexpr float fSQRT_3_OVER_3 = 0.57735026919f;
//-----------------------------------------------------------------------------------------------
...
float Compute1dPerlinNoise( float position, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		int indexWest = (int) positionFloor;
		int indexEast = indexWest + 1;
		float gradientWest = gradients[ Get1dNoiseUint( indexWest, seed ) & 0x00000001 ]; // Index at
		float gradientEast = gradients[ Get1dNoiseUint( indexEast, seed ) & 0x00000001 ]; //  0 or 1
...
float Compute2dPerlinNoise( float posX, float posY, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		const Vec2& gradientSW = gradients[ noiseSW & 0x00000007 ]; // Indexes in 0-7
		const Vec2& gradientSE = gradients[ noiseSE & 0x00000007 ]; //  for gradient
		const Vec2& gradientNW = gradients[ noiseNW & 0x00000007 ]; //  vector lookup
		const Vec2& gradientNE = gradients[ noiseNE & 0x00000007 ]; //  table [8]
...
float Compute3dPerlinNoise( float posX, float posY, float posZ, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	static const Vec3 gradients[ 8 ] = // Traditional "12 edges" requires modulus and isn't any better.
	{
		...
		Vec3( +fSQRT_3_OVER_3, -fSQRT_3_OVER_3, +fSQRT_3_OVER_3 ), //  corners, so xyz components
		Vec3( -fSQRT_3_OVER_3, -fSQRT_3_OVER_3, +fSQRT_3_OVER_3 ), //  are each sqrt(3)/3, i.e.
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		Vec3 gradientBelowSW = gradients[ noiseBelowSW & 0x00000007 ]; // Get a 0-7 number
		Vec3 gradientBelowSE = gradients[ noiseBelowSE & 0x00000007 ]; //  from raw noise
		Vec3 gradientBelowNW = gradients[ noiseBelowNW & 0x00000007 ]; //  to determine
		Vec3 gradientBelowNE = gradients[ noiseBelowNE & 0x00000007 ]; //  vector gradient 
		Vec3 gradientAboveSW = gradients[ noiseAboveSW & 0x00000007 ]; //  table lookup
		Vec3 gradientAboveSE = gradients[ noiseAboveSE & 0x00000007 ]; //  index in [8]
...
float Compute4dPerlinNoise( float posX, float posY, float posZ, float posT, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	static const Vec4 gradients[ 16 ] = // Hard to tell if this is any better in 4D than just having 8
	{
		...
		Vec4( -0.5f, +0.5f, +0.5f, +0.5f ), //  of the 16 hypercube corners, so xyz components
		Vec4( +0.5f, -0.5f, +0.5f, +0.5f ), //  are each sqrt(4)/4, i.e. +/- one-half.
		Vec4( -0.5f, -0.5f, +0.5f, +0.5f ), // 
		Vec4( +0.5f, +0.5f, -0.5f, +0.5f ), // It's hard to tell whether these are any
		Vec4( -0.5f, +0.5f, -0.5f, +0.5f ), //  better or worse than vectors facing axes
		Vec4( +0.5f, -0.5f, -0.5f, +0.5f ), //  (1,0,0,0) or 3D edges (.7,.7,0,0) or 4D 
		Vec4( -0.5f, -0.5f, -0.5f, +0.5f ), //  edges (.57,.57,.57,0) but less-axial
		Vec4( +0.5f, +0.5f, +0.5f, -0.5f ), //  gradients looked a little better with 2D 
		Vec4( -0.5f, +0.5f, +0.5f, -0.5f ), //  and 3D noise so I'm assuming this is as 
		...
		Vec4( +0.5f, +0.5f, -0.5f, -0.5f ), //
		Vec4( -0.5f, +0.5f, -0.5f, -0.5f ), // Plus, we want a power-of-two number of
		Vec4( +0.5f, -0.5f, -0.5f, -0.5f ), //  gradients, so we can cheaply select one
		Vec4( -0.5f, -0.5f, -0.5f, -0.5f )  //  from bit-noise (use bit-mask, not modulus).
	};
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		Vec4 gradientBeforeBSW = gradients[ noiseBeforeBSW & 0x0000000F ]; // Get an index
		Vec4 gradientBeforeBSE = gradients[ noiseBeforeBSE & 0x0000000F ]; //  of 0-15 for
		Vec4 gradientBeforeBNW = gradients[ noiseBeforeBNW & 0x0000000F ]; //  gradient
		Vec4 gradientBeforeBNE = gradients[ noiseBeforeBNE & 0x0000000F ]; //  table [16]
		Vec4 gradientBeforeASW = gradients[ noiseBeforeASW & 0x0000000F ]; //  lookup index 
		Vec4 gradientBeforeASE = gradients[ noiseBeforeASE & 0x0000000F ]; //  based on
		Vec4 gradientBeforeANW = gradients[ noiseBeforeANW & 0x0000000F ]; //  raw noise.
