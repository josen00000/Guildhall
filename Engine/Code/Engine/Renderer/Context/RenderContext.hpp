...
#include "Engine/Core/Vertex/Vertex_PCUTBN.hpp"
...
public:
	...
	void StartEffect( Texture* dst, Texture* src, Shader* shader, RenderBuffer* ubo ); // temp use
	void StartBloomEffect( Texture* dst, Texture* src, Texture* color, Shader* shader, RenderBuffer* ubo ); // temp use
	void EndEffect();
	...
	// Accessor
	ID3D11Device* GetD3DDevice() const { return m_device; }
	ID3D11DeviceContext* GetD3DContext() const { return m_context; }
...
private:
	...
	// begin camera
	void SetCameraViewport( IntVec2 viewPortSize );
	void SetRenderTarget( Camera* camera );
...
//	RenderBuffer*	m_dissolveUBO		= nullptr;
	...
	BlendMode m_currentBlendMode = BLEND_ALPHA;
