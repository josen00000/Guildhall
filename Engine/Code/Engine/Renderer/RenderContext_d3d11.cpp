...
RenderContext_d3d11::RenderContext_d3d11()
	:RenderContext(RenderContextType::RENDER_CONTEXT_TYPE_D3D11)
{
}

void RenderContext_d3d11::StartUp( Window* window )
{
...
void RenderContext_d3d11::ShutDown()
{
...
void RenderContext_d3d11::BeginFrame()
{
	double deltaTime = (float)m_clock->GetLastDeltaSeconds();
	UpdateFrameTime( (float)deltaTime );
}

void RenderContext_d3d11::EndFrame()
{
	Prensent();
}

void RenderContext_d3d11::BeginCamera( Camera* camera, Convention convention/*=X_RIGHT_Y_UP_Z_BACKWARD*/ )
{
#if defined(RENDER_DEBUG)
	ClearState();
...
void RenderContext_d3d11::BeginCameraViewport( IntVec2 viewPortSize )
{
...
void RenderContext_d3d11::BeginCameraRTVAndViewport( Camera* camera )
{
...
void RenderContext_d3d11::EndCamera()
{
...
void RenderContext_d3d11::ClearState()
{
...
Texture* RenderContext_d3d11::CreateRenderTarget( IntVec2 texSize )
{
...
Texture* RenderContext_d3d11::CreateRenderTargetWithSizeAndData( IntVec2 texSize, void* data )
{
	D3D11_TEXTURE2D_DESC desc;
	desc.Width				= texSize.x;
	desc.Height				= texSize.y;
...
void RenderContext_d3d11::StartBloomEffect( Texture* dst, Texture* src, Texture* color, Shader* shader, RenderBuffer* ubo )
{
	m_effectCamera->SetColorTarget( dst );
	//m_effectCamera->DisableClearColor();
	BeginCamera( m_effectCamera, g_convention );
...
void RenderContext_d3d11::EndEffect()
{
	m_context->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
	m_context->Draw( 3, 0 );
	EndCamera( );
...
Texture* RenderContext_d3d11::AcquireRenderTargetMatching( Texture* texture )
{
...
void RenderContext_d3d11::ReleaseRenderTarget( Texture* tex )
{
...
void RenderContext_d3d11::BindTexture( Texture* texture, uint slot )
{
	Texture* tempTexture = const_cast<Texture*>(texture);
	if( tempTexture == nullptr ) {
		tempTexture = m_texDefaultColor;
...
void RenderContext_d3d11::SetAttenuation( int lightIndex, Vec3 atten, AttenuationType type )
{
...
void RenderContext_d3d11::InitialLights()
{
...
void RenderContext_d3d11::CreateDebugModule()
{
...
void RenderContext_d3d11::ReportLiveObjects()
{
	if( nullptr != m_debug ) {
...
void RenderContext_d3d11::DestroyDebugModule()
{
	if( nullptr != m_debug ) {
...
void RenderContext_d3d11::SetDiffuseTexture( Texture* tex, int index )
{
	if( tex == nullptr ) {
...
void RenderContext_d3d11::SetNormalTexture( Texture* texture, int index )
{
	if( texture == nullptr ) {
...
void RenderContext_d3d11::SetMaterialTexture( Texture* texture, int index /*= 0 */ )
{
	BindTexture( texture, TEXTURE_USER_SLOT1 + index );
}

void RenderContext_d3d11::SetFog( float fogFarDist, float fogNearDist, Rgba8 fogFarColor, Rgba8 fogNearColor )
{
...
void RenderContext_d3d11::DisableFog()
{
...
void RenderContext_d3d11::BindSampler( const Sampler* sampler )
{
...
void RenderContext_d3d11::BindShader( Shader* shader )
{
...
void RenderContext_d3d11::BindShader( const char* fileName )
{
	Shader* shader = GetOrCreateShader( fileName );
	BindShader( shader );
}

void RenderContext_d3d11::BindShader( std::string fileName )
{
	BindShader( fileName.c_str() );
}

void RenderContext_d3d11::BindShaderState( ShaderState* state )
{
	state->PrepareForDraw();
}

void RenderContext_d3d11::BindRasterState()
{
...
void RenderContext_d3d11::BindVertexBuffer( VertexBuffer* vbo )
{
	// interlaced format
...
void RenderContext_d3d11::BindMaterial( Material* matl )
{
...
void RenderContext_d3d11::SetBlendMode( BlendMode blendMode )
{
...
void RenderContext_d3d11::SetSpecularFactor( float factor )
{
	m_model.specularFactor = factor;
	m_modelHasChanged = true;
}

void RenderContext_d3d11::SetSpecularPow( float pow )
{
	m_model.specularPow	 = pow;
	m_modelHasChanged = true;
}

void RenderContext_d3d11::SetModelAndSpecular( Mat44 model, float factor, float pow )
{
...
void RenderContext_d3d11::SetMaterialBuffer( RenderBuffer* ubo )
{
	BindUniformBuffer( UBO_MATERIAL_SLOT, ubo );
}

void RenderContext_d3d11::SetOffsetBuffer( RenderBuffer* ubo, int index )
{
	BindUniformBuffer( UBO_OFFSET_SLOT1 + index, ubo );
}

void RenderContext_d3d11::SetDissolveData( Vec3 startColor, Vec3 endColor, float width, float amount )
{
...
void RenderContext_d3d11::SetTintColor( Vec4 color )
{
...
void RenderContext_d3d11::SetTintColor( const Rgba8& color )
{
	Vec4 v_color;
...
void RenderContext_d3d11::EnableDepth( DepthCompareFunc func, bool writeDepthOnPass )
{
...
void RenderContext_d3d11::DisableDepth()
{
	// isdrawing();
	GUARANTEE_OR_DIE( m_currentCamera != nullptr, std::string( "current camera must not be empty." ) );
...
void RenderContext_d3d11::SetDiffuseAttenuation( int lightIndex, Vec3 atten )
{
	SetAttenuation( lightIndex, atten, ATTENUATION_DIFFUSE );
}

void RenderContext_d3d11::SetSpecularAttenuation( int lightIndex, Vec3 atten )
{
	SetAttenuation( lightIndex, atten, ATTENUATION_SPECULAR );
}

bool RenderContext_d3d11::CheckDepthStencilState( DepthCompareFunc func, bool writeDepthOnPass )
{
...
void RenderContext_d3d11::SetRasterCullMode( RasterCullMode mode )
{
...
void RenderContext_d3d11::SetRasterFillMode( RasterFillMode mode )
{
	if( m_rasterState == nullptr ) {
		m_device->CreateRasterizerState( m_defaultRasterStateDesc, &m_rasterState );
	}
...
void RenderContext_d3d11::SetFrontFaceWindOrder( RasterWindOrder order )
{
	if( m_rasterState == nullptr ) {
		m_device->CreateRasterizerState( m_defaultRasterStateDesc, &m_rasterState );
	}
...
void RenderContext_d3d11::DrawIndexed( int indexCount, int indexOffset /*= 0*/, int vertexOffset /*= 0 */ )
{
	UpdateLayoutIfNeeded();
	UpdateSceneIfNeeded();
	UpdateModelIfNeeded();
...
void RenderContext_d3d11::DrawMesh( GPUMesh* mesh )
{
...
void RenderContext_d3d11::DrawVertexVector( const std::vector<Vertex_PCU>& vertices )
{
	if( vertices.size() == 0 ) {
		return;
	}
...
void RenderContext_d3d11::DrawIndexedVertexVector( const std::vector<Vertex_PCU>& vertices, const std::vector<uint>& indexes )
{
...
void RenderContext_d3d11::DrawVertexArray( int vertexNum, Vertex_PCU* vertexes )
{
...
void RenderContext_d3d11::DrawPlane2D( Plane2 const& plane, float drawLength, Rgba8 tint )
{
...
void RenderContext_d3d11::DrawConvexHull( ConvexHull2 const& hull, Rgba8 tint )
{
...
void RenderContext_d3d11::DrawAABB2D( const AABB2& bounds, const Rgba8& tint, const Vec2& uvMin /*= Vec2::ZERO*/, const Vec2& uvMax /*= Vec2::ONE */ )
{
...
void RenderContext_d3d11::DrawAABB2DWithBound( const AABB2& bounds, const Rgba8& tint, float boundThick, const Rgba8& boundColor )
{
...
void RenderContext_d3d11::DrawPolygon2D( ConvexPoly2 polygon, Rgba8 tint )
{
...
void RenderContext_d3d11::DrawPolygon2DWithBound( ConvexPoly2 polygon, Rgba8 tint, float boundThick, Rgba8& boundColor )
{
...
void RenderContext_d3d11::DrawConvexPoly2D( ConvexPoly2 convPoly, Rgba8 tint )
{
...
void RenderContext_d3d11::DrawLine( const Vec2& startPoint, const Vec2& endPoint, const float thick, const Rgba8& lineColor )
{
	LineSegment2 tem = LineSegment2( startPoint, endPoint );
	DrawLine( tem, thick, lineColor );
}

void RenderContext_d3d11::DrawLine( const LineSegment2& lineSeg, float thick, const Rgba8& lineColor )
{
	//BindVertexBuffer( m_immediateVBO );
...
void RenderContext_d3d11::DrawLineWithHeight( const LineSegment2& lineSeg, float height, float thick, const Rgba8& lineColor )
{
	float halfThick = thick / 2;
	Vec2 direction = lineSeg.GetDisplacement();
	direction.SetLength( halfThick );
...
void RenderContext_d3d11::DrawCircle( Vec3 center, float radius, float thick, const Rgba8& circleColor )
{
	float degree = 0;
	float nextDegree = 0;
...
void RenderContext_d3d11::DrawFilledCircle( Vec3 center, float radiu, const Rgba8& filledColor )
{
	float degree = 0;
	float nextDegree = 0;
	const int vertexNum = 32;
...
void RenderContext_d3d11::DrawCylinder( const Cylinder3& cylinder, int level, const Rgba8& tintColor )
{
...
Texture* RenderContext_d3d11::GetSwapChainBackBuffer()
{
	return m_swapChain->GetBackBuffer();
}

BitmapFont* RenderContext_d3d11::CreateOrGetBitmapFontFromFile( const char* fontName, const char* fontFilePath )
{
...
Texture* RenderContext_d3d11::CreateOrGetTextureFromFile( const char* imageFilePath )
{
...
Texture* RenderContext_d3d11::CreateTextureFromColor( Rgba8 color )
{
	// get or create
	D3D11_TEXTURE2D_DESC desc;
	desc.Width				= 1;
...
Texture* RenderContext_d3d11::CreateTextureFromVec4( Vec4 input )
{
...
Shader* RenderContext_d3d11::GetOrCreateShader( char const* fileName )
{
...
SwapChain* RenderContext_d3d11::CreateSwapChain( Window* window, uint flags )
{
...
void RenderContext_d3d11::AddTexture( Texture* tex )
{
...
// private
void RenderContext_d3d11::UpdateLayoutIfNeeded()
{
	 //happen in both draw
...
void RenderContext_d3d11::UpdateSceneIfNeeded()
{
...
void RenderContext_d3d11::UpdateModelIfNeeded()
{
...
void RenderContext_d3d11::UpdateFrameTime( float deltaSeconds )
{
...
void RenderContext_d3d11::CreateBlendState()
{
...
void RenderContext_d3d11::createRasterState()
{
	D3D11_RASTERIZER_DESC desc;
	desc.FillMode				= D3D11_FILL_SOLID;
	desc.CullMode				= D3D11_CULL_NONE;
...
void RenderContext_d3d11::SetAmbientColor( Rgba8 color )
{
...
void RenderContext_d3d11::SetAmbientIntensity( float intensity )
{
	m_sceneData.ambient.SetW( intensity );
	m_sceneHasChanged = true;
}

void RenderContext_d3d11::SetAmbientLight( Rgba8 color, float intensity )
{
	SetAmbientColor( color );
	SetAmbientIntensity( intensity );
}

void RenderContext_d3d11::DisableAmbient()
{
	SetAmbientLight( Rgba8::WHITE, 1.f );
}

void RenderContext_d3d11::EnableLight( int index, const light_t& lightInfo )
{
	m_sceneData.lights[index] = lightInfo;
	m_sceneHasChanged = true;
}

void RenderContext_d3d11::EnablePointLight( int index, Vec3 position, Rgba8 color, float intensity, Vec3 attenuation )
{
...
void RenderContext_d3d11::EnableDirectionLight( int index, Vec3 position, Rgba8 color, float intensity, Vec3 attenuation )
{
	light_t& light = m_sceneData.lights[index];
	light.position = position;
	light.color.SetXYZ( color.GetVec3Color() );
...
void RenderContext_d3d11::EnableSpotLight( int index, Vec3 position, Rgba8 color, float intensity, Vec3 attenuation, Vec3 direction, float halfInnerDegrees, float halfOuterDegrees )
{
	light_t& light = m_sceneData.lights[index];
	light.position = position;
	light.color.SetXYZ( color.GetVec3Color() );
...
void RenderContext_d3d11::DisableLight( int index )
{
	m_sceneData.lights[index].color.SetW( 0.f );
	m_sceneHasChanged = true;
}

void RenderContext_d3d11::Prensent()
{
	m_swapChain->Present();
}

BitmapFont* RenderContext_d3d11::CheckBitmapFontExist( const char* imageFilePath ) const
{
...
BitmapFont* RenderContext_d3d11::CreateBitmapFontFromFile( const char* fontName, const char* fontFilePath )
{
...
Texture* RenderContext_d3d11::CreateTextureFromFile( const char* imageFilePath )
{
...
void RenderContext_d3d11::CreateCubeMapTexturesFromImages( const Image* src )
{
...
Texture* RenderContext_d3d11::CheckTextureExist( const char* imageFilePath ) const
{
...
void RenderContext_d3d11::CleanTextures()
{
...
void RenderContext_d3d11::CleanShaders()
{
...
void RenderContext_d3d11::CreateDefaultRasterStateDesc()
{
